                        ...
                        if ($user['User']['password'] === $hashed) {
                            if ($user['User']['active'] == 1) {
                                if ($user['User']['email_verified'] == 1) {
                                    ...
                                    $errorMsg = __('Your email has not been confirmed.<span onclick="sendlink(' . $user['User']['id'] . ',' . "'successmsg1'" . ')" style="color:#1F6CBA;cursor:pointer;">Resend Activation link</span> ', true);
                                }
                            } else {
...
// need to submit login for captcha validation
    ...
    public function logout($msg = true) {
        ...
        if ($msg) {
            $this->Session->setFlash(__('You are successfully signed out'), 'success');
        }
        $this->redirect(LOGOUT_REDIRECT_URL);
    }

    /*
     * method will check email on blur
     */
    ...
    public function checkemail() {
        ...
        if ($this->request->is('ajax')) {
            ...
            $user = $this->User->find('first', array('fields' => array('User.id'), 'conditions' => $conditions));
    ...
    /*
     * method to check password on blur
     * Will check for password for entered username
     */
    ...
    public function checkpassword() {
        ...
        if ($this->request->is('ajax')) {
            if (!empty($this->request->data['User']['email'])) {
                ...
                $password = $this->request->data['User']['password'];
                $user = $this->User->find('first', array('fields' => array('User.id', 'User.salt', 'User.password', 'User.active', 'User.email_verified'), 'conditions' => array('OR' => array('User.username' => $uname, 'User.email' => $uname))));
                if ($user) {
                    $hashed = $this->UserAuth->makePassword($password, $user['User']['salt']);
                    if ($user['User']['password'] === $hashed) {
                        echo "true";
    ...
    /*
     * method will check username on blur
     */
    ...
    public function checkusername() {
        ...
        if ($this->request->is('ajax')) {
            $uname = $this->request->data['User']['email'];
            $user = $this->User->find('first', array('conditions' => array('OR' => array('User.username' => $uname, 'User.email' => $uname), 'NOT' => array('User.user_group_id' => 1))));
            if (isset($user['User']['id'])) {
                echo "true";
            } else {
                echo "false";
        ...
        if (isset($_GET['ident']) && isset($_GET['activate'])) {
            ...
            if (!empty($user)) {
                if (!$user['User']['email_verified']) {
                    ...
                    $this->Session->setFlash(__('Thank you, your account is already activated'), 'success');
                }
            } else {
                $this->Session->setFlash(__('Sorry something went wrong, please click on the link again'), 'danger');
            }
        } else {
            $this->Session->setFlash(__('Sorry something went wrong, please click on the link again'), 'danger');
        }
        ...
        if ($this->request->is('ajax')) {
            ...
            if ($this->User->LoginValidate()) {
                ...
                if (empty($user)) {
                    $user = $this->User->findByEmail($email);
                    if (empty($user)) {
                        return json_encode(array('error' => 1, 'message' => '<div class="alert alert-danger">Incorrect Email</div>'));
                    }
                }
                // check for unverified account
                if ($user['User']['id'] != 1 and $user['User']['email_verified'] == 0) {
                    return json_encode(array('error' => 1, 'message' => '<div class="successmsg"><div class="alert alert-danger">Your email has not been confirmed.<span onclick="sendlink(' . $user['User']['id'] . ',' . "'successmsg'" . ')" style="color:#1F6CBA;cursor:pointer;">Resend Activation link</span></div></div>'));
                }
                $this->User->sendForgotPasswordMail($user);
                return json_encode(array('error' => 1, 'message' => '<div class="alert alert-success">Please check your email for reset your password</div>'));
                $this->Session->setFlash(__('Please check your email for reset your password'), 'success');
            }
        }
    }
